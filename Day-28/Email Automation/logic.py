...
SMTP_PORT = 587
SENDER_EMAIL = "narasimhareddybaireddy2004@gmail.com"
SENDER_PASSWORD  = "egag mdpg xelw ryaw"
...
def  send_email(to_email, subject, body,attachement=None):
    try:
        ...
        if attachement:
            for file_path in attachement:
                if os.path.exists(file_path):
                    with open(file_path, "rb") as f:
                        mime_base = MIMEBase("application", "octect-stream")
                        mime_base.set_payload(f.read())
                        encoders.encode_base64(mime_base)
                        mime_base.add11_header(
                            "content-Disposition",
